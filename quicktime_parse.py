...
_IGNORE_ATOMS = ["iods", "mdat"] # undocumented atoms or redundant for metadata
_ATOMS = {
	"pnot": (12, "I2x4s2x",
...
class Mov(object):
	...
	def _parse_atom(self, atom, length, depth, variable=False, chained=False):
		...
		for i in range(0, len(k)):
			...
			if not metakey in self.metadata.keys():
	...
	def _parse_udta(self, length, depth):
		...
		while n < length:
			...
			atom_size = struct.unpack(">I", self._f_read(4))[0]
			if atom_size == 0:
				return
			data_type = struct.unpack(">4s", self._f_read(4))[0]
